# 0-L2M-download-2023-24:
#  Download all the 2023-24 L2M reports in raw form to then evaluate:
...
url_l2m <- paste0("https://official.nba.com/",
                  "2023-24-nba-officiating-last-two-minute-reports/")
  ...
  if (is.data.frame(l2m_site)) {
    ...
    data2 <- data1 %>% 
      ...
      mutate(period = str_remove(period, "Period:"),
             ...
             committing = str_remove(player,
                                     "Player:"),
             disadvantaged = str_remove(opponent,
                                        "Opponent:"),
...
write_csv(corrections, paste0(local_dir, "/scraped_202324.csv"))
write_rds(corrections, paste0(local_dir, "/scraped_202324.rds"))
