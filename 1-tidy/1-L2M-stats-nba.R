...
odd_stats_dic        <- odd_stats$player_team
names(odd_stats_dic) <- odd_stats$player_name

# # Box scores from stats
# stats_games <- read_rds("0-data/bkref/bkref_box.rds") %>% 
...
#          player_name = ifelse(is.na(bad_stat_names[player_name]),
#                               player_name, bad_stat_names[player_name]),
...
odd_stats_c <- rename(odd_stats,
                      committing = player_name,
                      committing_team = player_team)
odd_stats_d <- rename(odd_stats,
                      disadvantaged = player_name,
...
l2m_games_stats <- 
  ...
  mutate(committing_team = ifelse(is.na(odd_stats_dic[committing]),
                                  committing_team, odd_stats_dic[committing]),
         disadvantaged_team = ifelse(is.na(odd_stats_dic[disadvantaged]),
                                     disadvantaged_team,
                                     odd_stats_dic[disadvantaged]),
...
l2m_games_stats2 <- l2m_games_stats %>% 
...
# new_season <- filter(l2m_games_stats, date > "2019-10-10")
# new_season %>% 
...
write_rds(l2m_games_stats3, paste0(local_dir, "/L2M-stats-nba.rds"))
