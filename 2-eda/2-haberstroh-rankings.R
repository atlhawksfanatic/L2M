...
ref_rate_szn <- refs %>% 
  select(game_id, season, official1, official2, official3) %>% 
  pivot_longer(c(-game_id, -season)) %>% 
  filter(!is.na(value)) %>% 
  ...
  group_by(season, official = value) %>% 
...
season_calc <- team_calc %>% 
  ...
  mutate(szn_type = case_when(str_sub(season, 1, 1) == "1" ~ "pre-season",
                              ...
                              T ~ NA_character_),
         szn = as.numeric(str_sub(season, 2))) %>% 
  filter(szn_type == "regular season") %>% 
  select(szn, team, ref_rating, tv_games, games)
...
write_csv(season_calc, "2-eda/L2M/figures/haberstroh_ref_rankings.csv")
